...
void *plane_thread(void *arg) {
    ...
    unsigned int min_time = 50, max_time = 100000;
    while (1) {
        printf("%d | Plane #%d is in the air.\n", on_aircraft_carrier, plane_id);
        usleep(random_utime(min_time, max_time));
        printf("%d | Plane #%d is going to land.\n", on_aircraft_carrier, plane_id);
        land();
        printf("%d | Plane #%d has landed.\n", on_aircraft_carrier, plane_id);
        usleep(random_utime(min_time, max_time));
        printf("%d | Plane #%d is going to start.\n", on_aircraft_carrier, plane_id);
        start();
        printf("%d | Plane #%d has started.\n", on_aircraft_carrier, plane_id);
    }
